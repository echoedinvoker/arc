...
class Item {
    constructor(name, orders, color, id) {
        this.name = name;
        this.orders = orders;
        this.color = color;
        this.id = id;
...
class ItemModel {
    constructor(data = [], id = []) {
        this.menuMap = new Map(data.map((d, i) => [id[i], (d === null || d === void 0 ? void 0 : d.color) ? new Item(d.name, d.orders, d.color) : new Item(d.name, d.orders)]));
    }
    get getMenu() {
        return this.menuMap.values();
    }
    get getMax() {
        return max$1(this.getMenu, d => d.orders) || 0;
    }
    get getNames() {
        return Array.from(this.getMenu).map(item => item.name);
    }
    set(data, id) {
        this.menuMap.set(id, (data === null || data === void 0 ? void 0 : data.color) ? new Item(data.name, data.orders, data.color) : new Item(data.name, data.orders));
    }
    remove(id) {
...
class ScaleGenerator {
    constructor(range, length) {
        this.r = linear()
            .range(range);
        this.x = band()
            .range([length, 0])
            .padding(0.1);
        this.c = ordinal(category10);
    }
    update(items) {
...
function createRadialAxis(scale, radius, config) {
    return function (g) {
...
function createArcAxis(scaleGenerator, config) {
    return (selection) => {
...
class AxisRenderer {
    constructor(arcAxisGroup, namesGroup, radius, config, itemModel, scale) {
        this.arcAxisGroup = arcAxisGroup;
        this.namesGroup = namesGroup;
        this.config = config;
    ...
    update() {
        ...
        const namesAxis = axisBottom(scaleXCopied);
        this.arcAxisGroup
...
class ArcBarRenderer {
    constructor(group, config, scale) {
        this.group = group;
        this.config = config;
        this.scale = scale;
        this.cbStartAngle = () => this.scale.r(0) * Math.PI / 180;
        this.cbEndAngle = (d) => this.scale.r(d.orders) * Math.PI / 180;
        this.cbEndAngleTween = (orders) => this.scale.r(orders) * Math.PI / 180;
        this.cbMiddleAngle = (d) => (this.cbStartAngle() + this.cbEndAngle(d)) / 2;
    ...
    update(items) {
        ...
        arcs
            ...
            .on('end', function (d) {
            select(this).attr('data-prev-d', d.orders);
        });
        ...
        texts.enter().append('text')
            ...
            const i = interpolate$1(0, d.orders);
            return (t) => `rotate(${(this.cbMiddleAngleTween(i(t)) * 180 / Math.PI)})`;
        })
            .on('end', function (d) {
            select(this).attr('data-prev-angle', d.orders);
        });
